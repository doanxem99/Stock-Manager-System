...
# plotly subplots is used to create multiple plots in one figure
...
def _vnquant_candle_stick_source(title,
                                symbol,
                                 start_date, end_date,
                                 colors=['blue', 'red'],
...
def vnquant_candle_stick_source(
    ...
    data.columns = ['high', 'low', 'open', 'close', 'adjust', 'volume']
    title = '{} stock price from {} to {}'.format(symbol, start_date, end_date)
    ...
    fig = make_subplots(rows=num_indices + 1, cols=1, shared_xaxes=True, vertical_spacing=0.1,
    ...
    if graph_json is None:
        graph_json = json.dumps({'error': 'Data not found'})
    return graph_json
...
def vnquant_candle_stick(data,
    ...
    if isinstance(data, str):
        return vnquant_candle_stick_source(symbol=data, start_date=start_date, end_date=end_date,
